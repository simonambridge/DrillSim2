  ...
  Begin
    ...
    Begin
      if abs(Hole[MaxHoles,1]-BitTD) >= 1
        then KellyHeight:=KellyHeight - 0.1
        else KellyHeight:=KellyHeight - 0.01;    { max decrement when less than 1 foot off bottom }
...
begin
  ...
  Begin
    ...
    Begin
      if abs(Hole[MaxHoles,1]-BitTD) >= 1
        then KellyHeight:=KellyHeight + 0.1
        else KellyHeight:=KellyHeight + 0.01;    { max increment when less than 1 foot off bottom }
...
begin
  With Data do              {only allowed when on slips }
  Begin
...
begin
  ...
  BushingImage.Picture.LoadFromFile('kellybushingup-0.png');
