...
begin
  if Simulating then With Data do              { - ChokeValue aperture }
  Begin
    ChokeValue:=ChokeValue - 1;
    if ChokeValue < 5 then
    Begin
      if ChokeValue=4 then CloseChoke;
      MessageToMemo(65);
      ChokeValue:=Zero;
      DeltaCsgPr:=DeltaCsgPr + PlChoke;
      PlChoke:=Zero;
    End;
    ChokeValue.Caption:=FloatToStr(ChokeValue);
    StringToMemo('Choke : ' + FloatToStr(ChokeValue));
  end;
End;
...
begin
  ...
  Begin
    ...
    if ChokeValue>100 then ChokeValue:=100;
    ChokeValue.Caption:=FloatToStr(ChokeValue);
    StringToMemo('Choke : ' + FloatToStr(ChokeValue));
  End;
End;
...
begin
  ...
  Begin
    ...
    Begin                           { call inits in DrillSimDataResets }
      ...
      SetSurfControls;                { set RAMs and ChokeValue line            }
