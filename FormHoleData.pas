...
begin
  if not ((TEdit(Sender).Text = '-')
      or (TEdit(Sender).Text = DefaultFormatSettings.DecimalSeparator)
      or (TEdit(Sender).Text = ''))
          then _WellCasingID:=Round2(StrToFloat(CasingIDData.Text)/UoMConverter[8],4); { inches }
end;
...
begin
  if not ((TEdit(Sender).Text = '-')
      or (TEdit(Sender).Text = DefaultFormatSettings.DecimalSeparator)
      or (TEdit(Sender).Text = ''))
          then _WellHole[1,2]:=Round2(StrToFloat(OHSection1IDData.Text)/UoMConverter[8],4); { inches }
end;
...
begin
  if not ((TEdit(Sender).Text = '-')
      or (TEdit(Sender).Text = DefaultFormatSettings.DecimalSeparator)
      or (TEdit(Sender).Text = ''))
          then _WellHole[2,2]:=Round2(StrToFloat(OHSection2IDData.Text)/UoMConverter[8],4); { inches }
end;
...
begin
  if not ((TEdit(Sender).Text = '-')
      or (TEdit(Sender).Text = DefaultFormatSettings.DecimalSeparator)
      or (TEdit(Sender).Text = ''))
          then _WellHole[3,2]:=Round2(StrToFloat(OHSection3IDData.Text)/UoMConverter[8],4); { inches }
end;
...
var Error : boolean;
  ...
  Begin
    ...
    if ((_WellHole[1,1] <= _WellCasingTD) and (Data.Casing=True)) or     { total OH depth must be greater than casing if present }
       ((_WellHole[1,1] <= _WellLinerBottomTD) and (Data.Liner=True)) or { total OH depth must be greater than liner if present }
       ((_WellHole[1,1] >= _WellHole[2,1]) and (_WellMaxHoles>1))        { total OH depth must be less than OH 2 depth if 2 OHs }
    then
    Begin
      ...
      StringToMemo('FormHoleData.Save: Error: Hole section 1 depth must be deeper than casing shoe and liner shoe (if present) and less than OD 2 TD (if present)');
    ...
    Begin
      ...
      StringToMemo('FormHoleData.Save: Error: Hole section 1 ID must be greater than zero, less than casing ID (if present), and greater than liner OD (if present)');
  ...
  Begin
    ...
    if (_WellHole[2,1] <= _WellHole[1,1]) or                             { total OH depth must be greater than previous OH TD }
       ((_WellHole[2,1] <= _WellLinerBottomTD) and (Data.Liner=True)) or { total OH depth must be greater than liner if present }
       ((_WellHole[2,1] >= _WellHole[3,1]) and (_WellMaxHoles>2))        { total OH depth must be less than OH 3 depth if 3 OHs }
    then
    Begin
      ShowMessage('Hole section 2 depth must be deeper than OH section 1 TD and liner shoe (if present) and less than OD 3 TD (if present)');
      StringToMemo('FormHoleData.Save: Error: Hole section 2 depth must be deeper than OH section 1 TD and liner shoe (if present) and less than OD 3 TD (if present)');
    ...
    Begin
      ...
      StringToMemo('FormHoleData.Save: Error: Hole section 2 ID must be greater than zero, less than OH section 1 ID, greater than OH 3 ID (present) and greater than liner OD (if present)');
  ...
  Begin
    ...
    if (_WellHole[3,1] <= _WellHole[2,1]) or                          { total OH depth must be greater than previous OH TD }
       ((_WellHole[3,1] <= _WellLinerBottomTD) and (Data.Liner=True)) { total OH depth must be greater than liner if present }
    then
    Begin
    ...
    if (_WellHole[3,2] >= _WellHole[2,2]) or                          { OD 3 ID must be less than OH 2 ID }
       ((_WellHole[3,2] <= _WellLinerID) and (Data.Liner=True))  or   { OH 3 ID must be greater than liner ID if present }
       (_WellHole[3,2] <= 0)                                          { OH 3 ID must be greater than zero }
    then
    Begin
      ShowMessage('Hole section 3 ID must be greater than zero, less than OH section 2 ID, and greater than liner OD (if present)');
      StringToMemo('FormHoleData.Save: Error: Hole section 3 ID must be greater than zero, less than OH section 2 ID (if present), and greater than liner OD (if present)');
  ...
  Begin
    ...
    if (_WellCasingTD <= Data.WaterDepth)     { Casing depth must be greater than Water Depth }
    then
    Begin
    ...
    if (_WellCasingID > Data.RiserID) or                             { Casing ID must be less than or equal to Riser ID }
       ((_WellCasingID <= _WellHole[1,2]) and (Data.MaxHoles>0))  { Casing ID must be greater than OH 1 ID if present }
    then
    Begin
      ShowMessage('Casing ID must be greater than zero, less than Riser ID, and greater than OH 1 OD (if present)');
      StringToMemo('FormHoleData.Save: Error: Casing ID must be greater than zero, less than Riser ID, and greater than OH 1 OD (if present)');
  ...
  Begin
    ...
    if (_WellLinerTopTD >= Data.CasingTD)     { Liner top depth must be less than Casing Shoe depth }
    then
    Begin
    ...
    if (_WellLinerBottomTD < Data.CasingTD)     { Liner bottom depth must be greater than Casing Shoe depth }
    then
    Begin
    ...
    if (_WellLinerID > Data.CasingID)         { Liner ID must be less than Casing ID }
    then
    Begin
...
begin
  ...
  StringToMemo('Checking Casing...');
    if CasingisOK = True then StringToMemo('Casing profile validation: Success') else
    Begin
  ...
  Begin
    ...
    if LinerisOK = True then StringToMemo('Liner profile validation: Success') else
    Begin
  ...
  Case Data.MaxHoles of    { very ugly...cant interate through objects but it works...}
    1 : Begin
          ...
          if WellSection1isOK = False then Error:=True;
        End;
    2 : Begin
          StringToMemo('Checking 2 OH sections...');
          if (WellSection1isOK = False)
          or (WellSection2isOK = False) then Error:=True;
        End;
    3 : Begin
          StringToMemo('Checking 3 OH sections...');
          if (WellSection1isOK = False)
          or (WellSection2isOK = False)
          or (WellSection3isOK = False) then Error:=True;
        End;
  End;
  if Error=False then StringToMemo('Hole profile validation: Success') else
  Begin
